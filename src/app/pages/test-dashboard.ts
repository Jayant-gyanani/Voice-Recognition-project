...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-zinc-50 font-sans flex flex-col">
      <header class="bg-zinc-900 text-white border-b border-zinc-800 px-8 py-4 flex items-center justify-between sticky top-0 z-10">
        <div class="flex items-center gap-2">
          <mat-icon class="text-indigo-400">science</mat-icon>
      ...
      <main class="flex-1 max-w-7xl w-full mx-auto px-8 py-12 flex flex-col">
        <div class="mb-8">
          <h1 class="text-3xl font-bold text-zinc-900">Voice Authentication Test</h1>
          <p class="text-zinc-500">Simulate the user experience of voice authentication and view the API response.</p>
        </div>

        <div class="grid md:grid-cols-2 gap-8 flex-1">
          <!-- Left Side: Voice Submission -->
          <div class="flex flex-col h-full">
            <div class="bg-white rounded-2xl border border-zinc-200 shadow-sm p-8 flex-1 flex flex-col">
              <div class="text-center mb-8">
                <div class="w-16 h-16 bg-indigo-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
                  <mat-icon class="text-indigo-600 text-3xl">record_voice_over</mat-icon>
                </div>
                <h2 class="text-2xl font-bold text-zinc-900 mb-2">Voice Authentication</h2>
                <p class="text-zinc-600">Please read the phrase below clearly to verify your identity.</p>
              </div>

              <div class="bg-zinc-50 rounded-xl p-6 border border-zinc-200 text-center mb-8 flex-1 flex flex-col justify-center">
                <p class="text-sm text-zinc-500 uppercase tracking-wider font-semibold mb-4">Verification Phrase</p>
                <p class="text-xl font-medium text-zinc-900 italic mb-8">"My voice is my password, and it verifies my identity."</p>
                
                <div class="relative w-32 h-32 mx-auto mb-6">
                  @if (isRecording()) {
                    <div class="absolute inset-0 bg-red-100 rounded-full animate-ping opacity-75"></div>
                    <div class="absolute inset-2 bg-red-200 rounded-full animate-pulse"></div>
                  }
                  <button 
                ...
                <p class="text-sm font-medium" [class.text-red-500]="isRecording()" [class.text-zinc-500]="!isRecording()">
                  @if (processing()) {
                    Processing audio...
                  } @else if (isRecording()) {
          ...
          <div class="flex flex-col h-full">
            <div class="bg-zinc-900 rounded-2xl border border-zinc-800 shadow-sm p-6 flex-1 flex flex-col text-white">
              ...
              <div class="flex-1 bg-black/50 rounded-xl border border-zinc-800 p-4 overflow-auto font-mono text-sm">
                @if (processing()) {
                  <div class="h-full flex flex-col items-center justify-center text-zinc-500">
                    <mat-icon class="animate-spin text-indigo-500 text-4xl mb-4">autorenew</mat-icon>
