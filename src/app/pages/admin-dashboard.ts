                ...
                @if (processing()) {
                  ...
                  <div class="flex-1 flex flex-col gap-4">
                    ...
                    @if (detectResult()?.status === 'success') {
                      <div class="p-4 rounded-xl border bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800/30 text-emerald-800 dark:text-emerald-400 flex items-center gap-3">
                        <mat-icon>check_circle</mat-icon>
                        <div>
                          <p class="font-bold">Recognised</p>
                          <p class="text-sm">{{ detectResult()?.message }}</p>
                        </div>
                      </div>
                    } @else if (detectResult()?.status === 'access_denied') {
                      <div class="p-4 rounded-xl border bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800/30 text-amber-800 dark:text-amber-400 flex items-center gap-3">
                      ...
                      <div class="p-4 rounded-xl border bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800/30 text-red-800 dark:text-red-400 flex items-center gap-3">
                        <mat-icon>cancel</mat-icon>
                        <div>
                          <p class="font-bold">{{ detectResult()?.status === 'error' ? 'Error' : 'Not Recognised' }}</p>
                          <p class="text-sm">{{ detectResult()?.message }}</p>
                        </div>
                      </div>
                    }

                    <!-- User card shown on success -->
                    @if (detectResult()?.status === 'success' && detectResult()?.data) {
                      <div class="flex items-center gap-4 p-4 bg-zinc-50 dark:bg-zinc-800/50 rounded-xl border border-zinc-200 dark:border-zinc-700">
                        @if (detectResult()?.data?.image) {
                          <img [src]="detectResult()?.data?.image" class="w-16 h-16 rounded-full object-cover border-2 border-emerald-400 flex-shrink-0" />
                        } @else {
                          <div class="w-16 h-16 rounded-full bg-indigo-100 dark:bg-purple-900/30 flex items-center justify-center flex-shrink-0">
                            <mat-icon class="text-indigo-500 dark:text-purple-400 text-3xl">person</mat-icon>
                          </div>
                        }
                        <div class="text-sm space-y-0.5">
                          <p class="font-bold text-zinc-900 dark:text-zinc-100 text-base">{{ detectResult()?.data?.name }}</p>
                          <p class="text-zinc-500 dark:text-zinc-400">{{ detectResult()?.data?.email }}</p>
                          <p class="text-zinc-500 dark:text-zinc-400">ID: {{ detectResult()?.data?.uniqueId }}</p>
                          @if (detectResult()?.data?.DOB) {
                            <p class="text-zinc-500 dark:text-zinc-400">DOB: {{ detectResult()?.data?.DOB }}</p>
                          }
                          @if (detectResult()?.data?.gender) {
                            <p class="text-zinc-500 dark:text-zinc-400">Gender: {{ detectResult()?.data?.gender }}</p>
                          }
                          <p class="text-emerald-600 dark:text-emerald-400 font-medium">Confidence: {{ detectResult()?.data?.confidence }}</p>
                        </div>
                      </div>
                    }
                    ...
                    <!-- Raw JSON output -->
                    <div>
                      <h3 class="text-xs font-bold text-zinc-500 dark:text-zinc-400 mb-2 uppercase tracking-wider">Raw API Response</h3>
                      <div class="bg-zinc-900 dark:bg-zinc-950 rounded-xl p-4 overflow-auto max-h-48 border border-zinc-800">
                        <pre class="text-xs font-mono text-emerald-400 whitespace-pre-wrap">{{ detectResult() | json }}</pre>
                      </div>
                    </div>
